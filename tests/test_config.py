...
def test_missing_api_key_defaults_to_empty(clean_env):
    clean_env.setenv("EMBEDDING_PROVIDER", "local")
    m = _reload_config()
    cfg = m.get_config()
    assert cfg.deepseek_api_key in ("", None)
...
def test_negative_price_raises(clean_env):
    clean_env.setenv("DEEPSEEK_API_KEY", "sk-test")
    clean_env.setenv("DEEPSEEK_INPUT_PRICE_PER_1M", "-1")
    m = _reload_config()
